...
def pyramid(initialPossition,eigVals,eigVecs,mean,testImage,training):
        ...
        reader = readerTemp
        X1, initialPossition = Matching_Real(training,initialPossition*2,eigVals,eigVecs,np.multiply(X2,2),testImage,he,wi)
...
#        break;
    ...
    initialPossition = np.add(np.mean(split(reader[0]),axis=1),-5)
